...
:mod:`datetime2` --- Date and time types with broader calendar coverage
=======================================================================

.. module:: datetime2
   :synopsis: Date and time types with broader calendar coverage.
.. moduleauthor:: Francesco Ricciardi <ZZZZemail>

The :mod:`datetime2` module provides similar services to those of the
:mod:`datetime` module, giving the user a choice of many different 
calendars that can be used for formatted output and input parsing.

Although in general it will follow the :mod:`datetime` module structure,
it is not intended to be a drop-in replacement. This documentation will 
highlight the differences, so that developers will be able to understand 
whether :mod:`datetime2` can be used in their code without code changes.


ZZZZ Verificare sintassi e rivedere frase e terminologia
The focus of the :mod:`datetime2` module is on providing a flexible framework 
to:

* add new calendars, at import time and dynamically;
* add new time measurement systems, at import time and dynamically;
* use internationalization technologies;
* use the Unicode Locale Database at the largest possible extent;
* interface seamlessly with other Python modules (e.g. :mod:`calendar`, or
  :mod:`pytz` ZZZZ(timedelta con anni mesi giorni)), or include their
  functionalities in its submodules.

Available calendars and time measuring systems will be discovered at
import time. Additionally, registering new systems at run time is also possible.

ZZZZ Vediamo se usiamo la stessa terminologia
...
ZZZZ Vediamo se implementare i fusi orari nella stessa maniera
...
The :mod:`datetime2` module has two attributes::

ZZZZ Vedere se la sintassi RTF is corretta

.. :attr:`default_cal`, with initial value ``gregorian``, which describes the 
   default calendar to be used for date input and output.
   
.. :attr:`default_time`, with initial value ``western``, which describes the
   default time measurement system to be used for time input and output.

It is possible, before instanciating a date or time object, to change one or both
default attributes, so the new calendar or time meausrement system can be used to
assign the new object value. 

Below there is the list of the availabe calendars or time measurement systems.
...
The :mod:`datetime2` define four main types to handle dates, times and their 
differences. These four types are based on 

.. class:: date
   :noindex:

   An idealized date, with no notion of time or time zone. Various representations 
   of the date are available, the default being the Gregorian calendar (default 
   can be changed after module import). Attributes: :attr:`day`.
...
.. class:: timedelta(days=0, seconds=0, nanoseconds=0, microseconds=0, milliseconds=0, minutes=0, hours=0, weeks=0)
   ...
   * A millisecond is converted to 1000000 nanoseconds.
   * A minute is converted to 60 seconds.
   * An hour is converted to 3600 seconds.
   * A week is converted to 7 days.

   and days, seconds and nanoseconds are then normalized so that the
   representation is unique, with

   * ``0 <= nanoseconds < 1000000000``
   * ``0 <= seconds < 86400`` (the number of seconds in one day)

   If any argument is a float and there are fractional nanoseconds, the fractional
   nanoseconds left over from all arguments are combined and their sum is rounded
   to the nearest nanosecond.  If no argument is a float, the conversion and
   ...
   example,
      ...
      >>> d = timedelta(nanoseconds=-1)
      >>> (d.days, d.seconds, d.nanoseconds)
      (-1, 86399, 999999999)

   Note that in :mod:`datetime2` the :class:`timedelta` class does not have a
   minimum or maximum value, contrary to the corresponding class in :mod:`datetime`,
   which has a minimum and a maximum value. 
...
+-----------------+--------------------------------------------+
| ``nanoseconds`` | Between 0 and 999999999 inclusive          |
+-----------------+--------------------------------------------+

Supported operations:

ZZZZ Verificare versione finale della documentazione ufficiale
.. XXX this table is too wide!

+--------------------------------+---------------------------------------------------+
| Operation                      | Result                                            |
+================================+===================================================+
| ``td1 = td2 + td3``            | Sum of *td2* and *td3*. Afterwards *td1*-*td2* == |
|                                | *td3* and *t1d*-*td3* == *t2d* are true.          |
+--------------------------------+---------------------------------------------------+
| ``td1 = td2 - td3``            | Difference of *td2* and *td3*. Afterwards *td1*   |
|                                | == *td2* - *td3* and *td2* == *td1* + *td3* are   |
|                                | true. (1)                                         |
+--------------------------------+---------------------------------------------------+
|``td1=td2 * f or td1 = f * td2``| TimeDelta multiplied by a float or an int.        |
|``td1=td2 * i or td1 = i * td2``| Afterwards *td1* / n == *td2* is true,            |
|                                | provided ``n != 0``.                              |
+--------------------------------+---------------------------------------------------+
...
.. method:: timedelta.total_seconds()
   ...
   Returns a float representing the total number of seconds contained in the duration.
...
.. literalinclude:: tzinfo-examples.py
